...
export default function ChangeThemeButton({ toggle }: ThemeButtonProps) {
  return (
    <div onClick={toggle} className="relative cursor-pointer">
      <div className="absolute rounded-full transition-all duration-300 size-5.5 bg-additional" />
      <div
      ...
      >
        <div
          className="absolute size-5.5 rounded-full duration-300 transition-all bg-background dark:bg-additional"
        />
      </div>
    </div>
  )
