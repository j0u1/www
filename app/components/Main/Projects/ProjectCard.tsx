...
export default function ProjectCard({ project }: { project: Projects }) {
  ...
  return (
    <Component
      ...
      key={project.title}
      className={`break-inside-avoid overflow-hidden rounded-xl outline outline-secondary/70 h-fit group/project inline-block select-none transition-all duration-300 w-full md:min-h-60.5 ${project.link ? 'cursor-pointer hover:brightness-125' : 'pointer-events-none'}`}
      href={project.link}
      target="_blank"
    >
